...
| `PATCH` | `/api/items/:id` | `{ done?, type?, title?, remindAt?, snoozeMinutes? }` — 완료 토글 / 분류 수정 / 알림 설정·스누즈·해제 |
...
- 🗓️ 반복 알림(매주 X), Resend 이메일 다이제스트, 캘린더(.ics) 내보내기
